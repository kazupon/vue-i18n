...
如果使用单文件组件构建Vue组件或Vue应用程序，则可以管理 `i18n` 自定义块的区域设置信息。
...
在上面，第一个自定义块使用 `src` 属性加载公共区域设置信息，第二个自定义块加载仅在单文件组件中定义的区域设置信息。这些区域设置信息将合并为组件的区域设置信息。
...
如果单个文件组件具有使用功能组件的模板，并且你已经定义了 `i18n` 自定义块，请注意你无法使用区域设置信息进行本地化。

例如，以下代码无法使用 `i18n` 自定义块的区域设置信息进行本地化。
